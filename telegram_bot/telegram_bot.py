...
def coordinates(message: types.Message):
    text = (
        'Введите через запятую широту (lat) и долготу (lon) числами.\n'
        ...
        'число не введено прогноз погоды будет показывать на 12 часов дня).\n'
        'Пример: 60, 50, 19\n'
        'Покажет погоду в точке с координатами lat=60, lon=50 на 19 часов.'
    )
    bot.send_message(message.chat.id, text)
...
def locations(message: types.Message):
    text = (
        ...
        'указания конкретной страны.\n'
        'Если хотите получить погоду в определённое время, введите, во '
        'сколько часов вы хотите получить прогноз погоды (целым числом, если '
        'число не введено прогноз погоды будет показывать на 12 часов дня).\n'
        'Пример: Нижний Новгород, Россия\n'
        'Покажет погоду в городе Нижний Новгород в России в 12 часов.'
    )
    bot.send_message(message.chat.id, text)
